...
local fmtDamage = function(entry)
	...
	local text = string.format("%s#DT#%s:%i:%i:%s:%s:%s:%s:%s", spellId, srcName or "Unknown", spellSchool, amount,
		overkill > 0 and overkill or "", resisted or "", blocked or "", absorbed or "",
		critical and "!" or glancing and "v" or crushing and "^" or "")
	if overkill > 0 then
...
local fmtHealing = function(entry)
	...
	return string.format("%i#HT#%s:%i:%s:%s", spellId, srcName, amount, overhealing > 0 and overhealing or "",
		critical and "!" or "")
end
local fmtDeBuff = function(entry)
	...
	return string.format("%i#A%s#%s:%s", spellId, (auraType == "DEBUFF") and "D" or "B", modifier,
...
local function unitDied(timestamp, playerID, playerName)
	...
	deathlog[0] = string.format("%s#%s#DEATH#%s:%s:%s:%s", playerName, class, _spellId or "", _srcName or "",
		_spellSchool or "", _amount or "")
...
-- COMBAT LOG EVENTS --
function collect.SPELL_DAMAGE(timestamp, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName,
							  spellSchool, amount, overkill, school, resisted, blocked, absorbed, critical, glancing,
							  crushing)
	...
	if dstFriend then
		...
		if addon.ids["deathlog"] then
			addDeathlogEvent(dstGUID, dstName, fmtDamage, timestamp, srcName, spellId, spellSchool, amount, overkill,
...
function collect.SWING_DAMAGE(timestamp, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, amount, overkill, school,
							  resisted, blocked, absorbed, critical, glancing, crushing)
	collect.SPELL_DAMAGE(timestamp, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, 0, "Melee", 0x01, amount,
		overkill, school, resisted, blocked, absorbed, critical, glancing, crushing)
end

function collect.ENVIRONMENTAL_DAMAGE(timestamp, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags,
									  environmentalType, amount, overkill, school, resisted, blocked, absorbed, critical,
									  glancing, crushing)
	collect.SPELL_DAMAGE(timestamp, "Environment", "Environment", srcFlags, dstGUID, dstName, dstFlags, environmentalType,
		environmentalType, 0x01, amount, overkill, school, resisted, blocked, absorbed, critical, glancing, crushing)
end

function collect.SPELL_MISSED(timestamp, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName,
							  spellSchool, missType, amountMissed)
...
function collect.SPELL_HEAL(timestamp, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName,
...
function collect.SPELL_DISPEL(timestamp, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName,
...
function collect.SPELL_INTERRUPT(timestamp, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName,
								 spellSchool, extraSpellID, extraSpellName, extraSchool)
	if addon.guidToClass[srcGUID] then
		EVENT("ir", srcGUID, srcName, dstName, extraSpellID, 1)
	end
end

function collect.SPELL_ENERGIZE(timestamp, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName,
								spellSchool, amount, powerType)
	if addon.guidToClass[dstGUID] then
...
function collect.SPELL_AURA_APPLIED_DOSE(timestamp, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId,
										 spellName, spellSchool, auraType, amount)
...
function collect.SPELL_AURA_REMOVED(timestamp, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName,
...
function collect.SPELL_RESURRECT(timestamp, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName,
								 spellSchool)
	if addon.ids["deathlog"] and addon.guidToClass[dstGUID] then
