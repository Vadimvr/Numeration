...
function a.CreteWindow:CreteWindow(window, name)
        ...
        reset:SetScript("OnMouseUp", function()
            ...
            for i = 1, GetNumDisplayChannels() do
                ...
                if category == "CHANNEL_CATEGORY_CUSTOM" then
                    tinsert(menuTable[2].menuList[7].menuList,
                        { text = name, arg1 = "CHANNEL", arg2 = channelNumber, func = reportFunction, notCheckable = 1 })
                end
            end
    ...
    function window:UpdateSegment(segment)
        if not segment then
            ...
            window.segment:SetText(segment)
            window.segment:Show()
        end
    end

    function window:SetTitle(name, r, g, b)
        window.title:SetVertexColor(r, g, b, s.titlealpha)
        window.titletext:SetText(name)
    end

    function window:GetTitle()
        return window.titletext:GetText()
    end
    ...
    function window:SetScrollPosition(curPos, maxPos)
        ...
        window.scroll:SetHeight(s.maxlines / maxPos * total)
        window.scroll:SetPoint("TOPLEFT", window.reset, "BOTTOMRIGHT", 2, -1 - (curPos - 1) / maxPos * total)
        window.scroll:Show()
    end
